...
void WorldSession::SendAuctionHello(ObjectGuid guid, Unit const* unit)
{
    ...
    AuctionHouseEntry const* ahEntry = AuctionHouseMgr::GetAuctionHouseEntryByFactionTemplateId(unit->GetFaction());
    if (!ahEntry)
        return;
...
void WorldSession::HandleAuctionSellItem(WorldPacket& recvData)
{
    ...
    AuctionHouseEntry const* auctionHouseEntry = AuctionHouseMgr::GetAuctionHouseEntryByFactionTemplateId(creature->GetFaction());
    ...
    if (itemsCount == 1 && item->GetCount() == count[0])
    {
        ...
        CharacterDatabaseTransaction trans = CharacterDatabase.BeginTransaction("WorldSession::HandleAuctionSellItem");
...
void WorldSession::HandleAuctionPlaceBid(WorldPacket& recvData)
{
    ...
    CharacterDatabaseTransaction trans = CharacterDatabase.BeginTransaction("WorldSession::HandleAuctionPlaceBid");
...
void WorldSession::HandleAuctionRemoveItem(WorldPacket& recvData)
{
    ...
    CharacterDatabaseTransaction trans = CharacterDatabase.BeginTransaction("WorldSession::HandleAuctionRemoveItem");
    if (auction && auction->owner == player->GetGUID().GetCounter())
    {
        ...
        if (pItem)
        {
            ...
            MailDraft(auction->BuildAuctionMailSubject(pItem, AUCTION_CANCELED), "")
                .AddItem(pItem)
...
void WorldSession::HandleAuctionListBidderItems(WorldPacket& recvData)
{
    ...
    AuctionHouseEntry const* ahEntry = AuctionHouseMgr::GetAuctionHouseEntryByFactionTemplateId(creature->GetFaction());
    if (!ahEntry)
        return;
