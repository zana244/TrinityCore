...
pAuraEffectHandler AuraEffectHandler[TOTAL_AURAS]=
{
    ...
    &AuraEffect::HandleModHitChance,                              // 54 SPELL_AURA_MOD_HIT_CHANCE implemented in Unit::MeleeSpellMissChance
                ...
                case FORM_DIREBEAR:
                {
                    ...
                    switch (GetMiscValue())
                    {
                        case FORM_CAT:
                        {
                            ...
                            if (target->IsPlayer())
                                target->ToPlayer()->UpdatePowerRegen(POWER_ENERGY);
...
void AuraEffect::HandleAuraModAttackPowerPercent(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    // GetAmount is 10 for 10% AP increase
    // target->GetTotalAuraMultiplier(SPELL_AURA_MOD_ATTACK_POWER_PCT) is 1.1 for 10% AP increase
...
void AuraEffect::HandlePeriodicDamageAurasTick(Unit* target, Unit* caster) const
{
    ...
    if (!GetSpellInfo()->HasAttribute(SPELL_ATTR4_FIXED_DAMAGE) && !GetSpellInfo()->HasAttribute(SPELL_ATTR0_CU_IGNORE_DMG_AVOIDANCE))
    {
