...
void GameEventMgr::StopEvent(uint16 event_id, bool overwrite)
{
    ...
    else if (serverwide_evt)
    {
        ...
        if (overwrite || data.state != GAMEEVENT_WORLD_FINISHED)
        {
            ...
            CharacterDatabaseTransaction trans = CharacterDatabase.BeginTransaction("GameEventMgr::StopEvent");
    ...
    if (itr != mQuestToEventConditions.end())
    {
        ...
        if (citr != mGameEvent[event_id].conditions.end())
        {
            ...
            if (citr->second.done < citr->second.reqNum)
            {
                ...
                CharacterDatabaseTransaction trans = CharacterDatabase.BeginTransaction("GameEventMgr::HandleQuestComplete");
...
{
    CharacterDatabaseTransaction trans = CharacterDatabase.BeginTransaction("GameEventMgr::SaveWorldEventStateToDB");
